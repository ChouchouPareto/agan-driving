...
import { BookOpenCheck, LoaderCircle, MessageCircleQuestion, Send } from "lucide-react";
...
export function AskWorkspace() {
  ...
  async function ask(event: FormEvent) {
    event.preventDefault(); const resumable = Boolean(questionId) && ["题目已确认，可以提交生成可信回答。", "问题已提交，可以重新发起回答。"].includes(status); if ((status && !resumable) || text.trim().length < 2) return; setError(""); setAnswer(null); setTicket(null); setStatus("正在准备问题…");
    try {
  ...
  return <><section className="hero"><h1>科目一，今天继续练</h1><p>先刷题巩固，遇到不懂的再问 AI 助教。</p></section><nav className="learningEntrances" aria-label="学习方式"><Link className="learningEntrance practiceEntrance" href="/practice"><span className="entranceIcon"><BookOpenCheck aria-hidden="true" size={24}/></span><span><strong>开始刷题</strong><small>从当前科目一题库练习</small></span><b>去练习 →</b></Link><a className="learningEntrance askEntrance" href="#ask-composer"><span className="entranceIcon"><MessageCircleQuestion aria-hidden="true" size={24}/></span><span><strong>AI 答疑</strong><small>输入或拍照上传看不懂的题</small></span></a></nav><section id="ask-composer" className="card questionComposer"><form onSubmit={ask}><label className="label" htmlFor="question">输入题目或科目一问题</label><textarea id="question" className="input" value={text} onChange={e => setText(e.target.value)} minLength={2} maxLength={2000} required/><div className="composerFooter"><OCRWorkspace initialTaskId={savedOCRTaskId} onTaskCreated={trackOCRTask} onQuestionCreated={useOCRQuestion}/><div className="composerActions"><span className="characterCount">{text.length}/2000</span><button className="button primary" type="submit" disabled={(Boolean(status) && !canResumeQuestion) || text.trim().length < 2}>{status && !canResumeQuestion ? <><LoaderCircle aria-hidden="true" className="spin" size={18}/>处理中</> : <><Send aria-hidden="true" size={18}/>提交问题</>}</button></div></div></form></section>{status && <div className="card status" aria-live="polite"><span className="dot"/>{status}</div>}{error && <div className="card error" role="alert">{error}</div>}{answer && <AnswerCard answer={answer} ticket={ticket} busy={Boolean(status)} onFeedback={feedback} onTicket={() => createReviewTicket()}/>}</>;
}
