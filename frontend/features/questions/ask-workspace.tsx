...
export function AskWorkspace() {
  ...
  useEffect(() => {
    ...
    api.getQuestion(savedQuestionId).then(detail => {
      setText(detail.text);
      setAnswer(detail.answer);
      setTicket(detail.ticket);
      setStatus(detail.answer || detail.ticket ? "" : "问题已提交，可以重新发起回答。");
    }).catch(reason => {
      setStatus("");
  ...
  return <div className="chatPage"><section className="chatStream" aria-label="与超级陪驾的对话"><div className="assistantMessage"><span className="assistantAvatar" aria-hidden="true"><Sparkles size={18}/></span><div><span className="messageAuthor">超级陪驾 · AI</span><p>嗨，我可以帮你讲题、识别题目图片，也可以直接带你开始刷题。</p><div className="promptSuggestions"><button onClick={()=>router.push("/practice")}>我要刷题</button><button onClick={()=>setText("直行车道可以右转吗？")}>问一道题</button></div></div></div>{questionId&&<div className="userMessage"><p>{text}</p></div>}{status&&<div className="assistantMessage compact" aria-live="polite"><span className="assistantAvatar" aria-hidden="true"><Sparkles size={16}/></span><div className="status"><span className="dot"/>{status}</div></div>}{error&&<div className="chatError" role="alert">{error}</div>}{answer&&<AnswerCard answer={answer} ticket={ticket} busy={isBusy} onFeedback={feedback} onTicket={()=>createReviewTicket()}/>}</section><section id="ask-composer" className="chatComposer"><div className={`toolTray ${toolsOpen?"open":""}`}><button className="toolTrayToggle" aria-expanded={toolsOpen} onClick={()=>setToolsOpen(value=>!value)}><BookOpenCheck aria-hidden="true" size={18}/><span>学习工具</span><ChevronDown aria-hidden="true" size={17}/></button>{toolsOpen&&<nav className="toolTrayLinks" aria-label="学习工具"><Link href="/practice"><BookOpenCheck aria-hidden="true" size={18}/>顺序刷题</Link><Link href="/practice?mode=wrong"><ListRestart aria-hidden="true" size={18}/>错题本</Link><Link href="/practice?mode=favorites"><Heart aria-hidden="true" size={18}/>收藏题</Link></nav>}</div><form className="chatInputBox" onSubmit={ask}><label className="srOnly" htmlFor="question">给超级陪驾发消息</label><textarea id="question" value={text} onChange={e=>setText(e.target.value)} placeholder="发消息、粘贴题目，或说‘我要刷题’" minLength={2} maxLength={2000} required/><div className="chatInputActions"><OCRWorkspace initialTaskId={savedOCRTaskId} onTaskCreated={trackOCRTask} onQuestionCreated={useOCRQuestion}/><span className="characterCount">{text.length}/2000</span><button className="chatSendButton" type="submit" aria-label="发送消息" disabled={isBusy||text.trim().length<2}>{isBusy&&!canResumeQuestion?<LoaderCircle aria-hidden="true" className="spin" size={19}/>:<Send aria-hidden="true" size={19}/>}</button></div></form><p className="aiDisclaimer">超级陪驾是 AI 助手，回答会引用当前题库依据。</p></section></div>;
}
