...
export function AskWorkspace() {
  ...
  return <><section className="hero"><h1>今天有什么没看懂？</h1><p>输入文字或上传题目图片；没有可靠依据时不会猜。</p></section><section className="card questionComposer"><form onSubmit={ask}><label className="label" htmlFor="question">输入题目或科目一问题</label><textarea id="question" className="input" value={text} onChange={e => setText(e.target.value)} minLength={2} maxLength={2000} required/><div className="composerMeta"><span>{text.length}/2000</span><button className="button primary" type="submit" disabled={(Boolean(status) && !canResumeQuestion) || text.trim().length < 2}>{status && !canResumeQuestion ? <><LoaderCircle aria-hidden="true" className="spin" size={18}/>处理中</> : <><Send aria-hidden="true" size={18}/>提交问题</>}</button></div></form><OCRWorkspace initialTaskId={savedOCRTaskId} onTaskCreated={trackOCRTask} onQuestionCreated={useOCRQuestion}/></section>{status && <div className="card status" aria-live="polite"><span className="dot"/>{status}</div>}{error && <div className="card error" role="alert">{error}</div>}{answer && <AnswerCard answer={answer} ticket={ticket} busy={Boolean(status)} onFeedback={feedback} onTicket={() => createReviewTicket()}/>}</>;
}
