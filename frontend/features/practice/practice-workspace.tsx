...
export function PracticeWorkspace({ initialMode = "all", license = "C1", subject = "subject-1" }: { initialMode?: Mode; license?: string; subject?: string }) {
  ...
  return <>
    ...
    {insights && <aside className="learningInsight"><div><span>超级陪驾建议</span><strong>{insights.recommendation}</strong></div>{insights.weak_points.length > 0 && <p>当前弱项：{insights.weak_points.map(item => item.name).join(" · ")}</p>}</aside>}
