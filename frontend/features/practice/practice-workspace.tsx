...
import { ArrowLeft, ArrowRight, CheckCircle2, Heart, ListFilter, MessageCircleQuestion, RotateCcw } from "lucide-react";
import { z } from "zod";
...
export function PracticeWorkspace({ initialMode = "all", license = "C1", subject = "subject-1" }: { initialMode?: Mode; license?: string; subject?: string }) {
  ...
  return <>
    ...
    <section className="practiceCompactBar" aria-label="练习进度与范围"><details><summary><ListFilter aria-hidden="true" size={17}/>{modeLabel}</summary><div className="practiceModeMenu" role="group" aria-label="练习范围">{([["all", "顺序练习"], ["wrong", "错题本"], ["favorites", "收藏题"]] as const).map(([value, label]) => <button key={value} className={mode === value ? "active" : ""} aria-pressed={mode === value} onClick={(event) => { setLoading(true); setError(""); setMode(value); event.currentTarget.closest("details")?.removeAttribute("open"); }}>{label}</button>)}</div></details>{summary && <p>已做 {summary.attempted} · 错题 {summary.wrong_count} · 收藏 {summary.favorite_count}</p>}</section>
